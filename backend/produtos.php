...
switch($_SERVER["REQUEST_METHOD"]){
    case "POST";
    $nomeProduto = $body['nome'];
    $nomeExistente = $produtos->selectByNome($nomeProduto);
    if (!empty($nomeExistente)) {
        echo json_encode(['status' => false, 'message' => 'Produtos já existe']);
    } else {
    ...
    case "GET":
        if (!isset($_GET['id'])) {
            ...
            if (!empty($resultado)) {
                ...
                echo json_encode(["status" => false, "produto" => null]);
            }
        }
        break;
